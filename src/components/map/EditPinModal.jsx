...
export default function EditPinModal({ event, onSave, onClose, pickingLocation, onStartPickLocation, groups = [] }) {
  ...
  // Migrate legacy pin on open so we always work with the new shape
  ...
  const [form, setForm] = useState({
    ...
    wikipedia_url: migrated.wikipedia_url ?? "",
  ...
  // Populate lat/lng when map pick completes
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    await onSave(event.id, {
      ...form,
      year,           // keep integer for timeline
      date: finalDate,
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-2xl p-5 w-[380px] max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            <label className="text-[10px] text-gray-500 block mb-0.5">Wikipedia URL</label>
            <input className="w-full border rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
              value={form.wikipedia_url} onChange={e => set("wikipedia_url", e.target.value)} />
          </div>

          {/* Group */}
          <div>
            <label className="text-[10px] text-gray-500 block mb-0.5">Group</label>
            <select className="w-full border rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white"
